...
const { createDir, createIndex } = require('./createIndex');
...
createFiles = (dir, template, files) => {
  ...
  switch (template) {
    ...
    case 'starter':
      console.log(template);
      createDir(dir).then(createIndex(dir)).then(createDat(dir))
      // importDat(dir, 'dat://start.archetype.cc/').then(deleteDat(dir)).then(createDat(dir))
      console.log("created starter folder");
      break;
    default:
      console.log(template);
      importDat(dir, template).then(deleteDat(dir)).then(createDat(dir))
